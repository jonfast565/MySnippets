...
def run_tests(limit=10000, num_tests=10, inc_val=1000):
    ...
    while start_val <= limit and list_size <= limit:
        ...
        for test in range(num_tests):
            ...
            thread_1 = threading.Thread(target=quicksort, args=(test_input_yes_t, first, br_2 - 1))
            thread_2 = threading.Thread(target=quicksort, args=(test_input_yes_t, br_2 + 1, br_1 - 1))
            thread_3 = threading.Thread(target=quicksort, args=(test_input_yes_t, br_1 + 1, br_3 - 1))
            thread_4 = threading.Thread(target=quicksort, args=(test_input_yes_t, br_3 + 1, last))
            ...
            # print threaded time
            print("Time elapsed (threaded):", num_seconds_t)

            # add to the threaded time
            ...
            print("Time elapsed (non-threaded):", num_seconds_t)
        ...
        # print averages
        print("Threaded average for", num_tests, "tests on list of size", list_size, "with start value of", start_val,
              "is", threaded_avg, ".")
        print("Non-threaded average for", num_tests, "tests on list of size", list_size, "with start value of",
              start_val, "is", non_threaded_avg, ".")
        print()

        # write all info to files (might want to write more, considering a lot more info)
        # first open
        file_1 = open('threaded_results.csv', 'a')
        file_2 = open('non_threaded_results.csv', 'a')
